-- Secure operator-assisted wallet payment during invoicing.
-- Keeps the client portal purchase flow unchanged.

create or replace function public.wallet_operator_purchase_invoice(
  p_client_id text,
  p_invoice_id text,
...
declare
  ...
  v_remaining_payment_method text;
...
begin
  ...
  if not exists (
    ...
    where s.id = p_session_id
      and s.actor_type = 'operator'
      and s.user_id = p_operator_user_id
      and s.revoked_at is null
      and s.expires_at > now()
  ) then
    raise exception 'Valid operator wallet session required';
  end if;

  if not exists (
    select 1
    from public.wallet_auth_sessions s
    where s.id = p_client_session_id
      and s.actor_type = 'client'
      and s.client_id = p_client_id
      and s.metadata->>'purpose' = 'operator_wallet_purchase'
      and s.metadata->>'invoice_id' = btrim(p_invoice_id)
      and s.metadata->>'operator_user_id' = p_operator_user_id
      and s.revoked_at is null
      and s.expires_at > now()
  ) then
    raise exception 'Valid client authorization required';
  end if;
  ...
  v_remaining_payment_method :=
    btrim(coalesce(p_remaining_payment_method, ''));

  if length(v_remaining_payment_method) > 80 then
    raise exception 'Remaining payment method is invalid';
  end if;
  ...
  if not found then
    ...
    loop
      ...
      if not coalesce(v_product.wallet_eligible, false) or coalesce(v_product.wallet_eligibility_status, '') <> 'eligible' then
    ...
    loop
      ...
      if not found or not coalesce(v_product.wallet_eligible, false) or coalesce(v_product.wallet_eligibility_status, '') <> 'eligible' then
        raise exception 'WALLET_PRODUCT_NOT_ELIGIBLE';
      end if;
    end loop;
  end if;
  ...
  from public.wallet_post_transaction(
    ...
    p_notes => 'Pago aplicado por operador autorizado desde el Bolsillo del cliente.',
    p_metadata => jsonb_build_object(
      'api_version', 4,
      'channel', 'operator_billing',
      'invoice_number', v_invoice.invoice_number
    )
  );
  ...
  if v_due_after = 0 then
    v_payment_status := 'Pagado';
    v_payment_method := 'Bolsillo';
  else
    if v_remaining_payment_method = '' then
      raise exception 'Remaining payment method is required for partial wallet payment';
    end if;

    v_payment_status := case
      when lower(v_remaining_payment_method) like '%cred%'
        then 'Pendiente'
      else 'Pagado'
    end;

    v_payment_method :=
      'Bolsillo + ' || v_remaining_payment_method;
  end if;
...
revoke all on function public.wallet_operator_purchase_invoice(
  text, text, text, jsonb, numeric, text, text, numeric,
  text, uuid, uuid, text, text, text
) from public, anon, authenticated;

grant execute on function public.wallet_operator_purchase_invoice(
  text, text, text, jsonb, numeric, text, text, numeric,
  text, uuid, uuid, text, text, text
) to service_role;

comment on function public.wallet_operator_purchase_invoice(
  text, text, text, jsonb, numeric, text, text, numeric,
  text, uuid, uuid, text, text, text
) is
  'Atomically creates an invoice and applies wallet funds through an authenticated operator with their own open shift.';
