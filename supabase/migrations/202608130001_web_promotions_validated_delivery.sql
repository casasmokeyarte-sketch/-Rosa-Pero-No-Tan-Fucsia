-- Promotions for online checkout and verified delivery addresses.
-- Existing promotions remain compatible; new scopes are web and first web purchase.
...
alter table public.discounts
  ...
  check (applies_to in ('todos', 'facturacion', 'domicilios', 'web', 'primera_compra_web'));
...
comment on column public.invoices.delivery_address_place_id is
  'Opaque Google Places identifier selected by the customer; it contains no balance or payment credential.';

comment on column public.invoices.delivery_address_verified is
  'True only when the checkout address was selected from the configured autocomplete provider.';
...
declare
  ...
  v_promo_discount numeric(14,2) := 0;
begin
  ...
  if not found then
    ...
    select coalesce(max(
      case
        when d.type = 'porcentaje' then
          least(v_subtotal, round(v_subtotal * least(100, greatest(0, d.value)) / 100, 2))
        else least(v_subtotal, greatest(0, d.value))
      end
    ), 0)
    ...
    where coalesce(d.active, false)
      and (
        d.applies_to in ('todos', 'web')
        or (
          d.applies_to = 'primera_compra_web'
          and not exists (
            select 1
            from public.invoices previous_invoice
            where previous_invoice.client_id = p_client_id
              and previous_invoice.cashier_name = 'Portal Online'
              and previous_invoice.payment_status <> 'Anulada'
          )
        )
      )
      and (d.start_date is null or timezone('America/Bogota', now())::date >= d.start_date)
      and (d.end_date is null or timezone('America/Bogota', now())::date <= d.end_date)
      and (d.start_time is null or timezone('America/Bogota', now())::time >= d.start_time::time)
      and (d.end_time is null or timezone('America/Bogota', now())::time <= d.end_time::time)
      and (
        d.active_days is null
        or cardinality(d.active_days) = 0
        or extract(dow from timezone('America/Bogota', now()))::integer = any(d.active_days)
      );

    -- Never stack promotions: apply only the largest valid benefit.
    v_discount := greatest(v_discount, v_promo_discount);
